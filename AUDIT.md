...
| ECG | Gadgetbridge has service `0x23/cmd=0x10`, DataSync package `hw.watch.health.ecganalysis`, file type `0x18` as `ecg_analysis`, and parser `HuaweiEcgFileParser.java` | `EcgMetaData`, `HiEcgMetaData`, ECG strings/routes exist in APK | Unsupported on current Band 10 artifact: no service `0x23`; ECG expand bits are now tracked explicitly | Revisit only if firmware advertises ECG bits/service or file type `0x18` succeeds |
