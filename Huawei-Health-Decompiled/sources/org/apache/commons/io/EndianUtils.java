...
public class EndianUtils {
    ...
    public static long readSwappedLong(byte[] bArr, int i) {
        ...
        return (((long) readSwappedInteger(bArr, i + 4)) << 32) + (((long) readSwappedInteger(bArr, i)) & Mp4Utils.UNSIGNED_INT_MAX_VALUE);
    }
    ...
    public static long readSwappedUnsignedInteger(byte[] bArr, int i) {
        validateByteArrayOffset(bArr, i, 4);
        return (((long) (bArr[i + 3] & UnsignedBytes.MAX_VALUE)) << 24) + (((long) ((bArr[i] & UnsignedBytes.MAX_VALUE) + ((bArr[i + 1] & UnsignedBytes.MAX_VALUE) << 8) + ((bArr[i + 2] & UnsignedBytes.MAX_VALUE) << 16))) & Mp4Utils.UNSIGNED_INT_MAX_VALUE);
    }

    public static long readSwappedUnsignedInteger(InputStream inputStream) throws IOException {
        return (((long) (read(inputStream) & 255)) << 24) + (((long) ((read(inputStream) & 255) + ((read(inputStream) & 255) << 8) + ((read(inputStream) & 255) << 16))) & Mp4Utils.UNSIGNED_INT_MAX_VALUE);
    }
