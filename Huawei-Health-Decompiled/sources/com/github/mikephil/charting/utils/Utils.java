...
public abstract class Utils {
    ...
    public static void drawXAxisValue(Canvas canvas, String str, float f, float f2, Paint paint, MPPointF mPPointF, float f3) {
        Paint.FontMetrics fontMetrics = mFontMetricsBuffer;
        ...
        if (f3 != 0.0f) {
            ...
            if (mPPointF.x != 0.5f || mPPointF.y != 0.5f) {
                FSize sizeOfRotatedRectangleByDegrees = getSizeOfRotatedRectangleByDegrees(rect.width(), fontMetrics2, f3);
            ...
            canvas.rotate(f3);
            canvas.drawText(str, fWidth - (fWidth2 * 0.5f), f4 - (fontMetrics2 * 0.5f), paint);
            canvas.restore();
        } else {
            if (mPPointF.x != 0.0f || mPPointF.y != 0.0f) {
                fWidth -= rect.width() * mPPointF.x;
