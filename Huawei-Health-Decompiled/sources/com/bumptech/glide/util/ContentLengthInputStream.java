...
public final class ContentLengthInputStream extends FilterInputStream {
    ...
    public int available() throws IOException {
        int iMax;
        synchronized (this) {
            iMax = (int) Math.max(this.contentLength - ((long) this.readSoFar), this.in.available());
        }
        return iMax;
    }
    ...
    public int read(byte[] bArr, int i, int i2) throws IOException {
        int iCheckReadSoFarOrThrow;
        synchronized (this) {
            iCheckReadSoFarOrThrow = checkReadSoFarOrThrow(super.read(bArr, i, i2));
        }
        return iCheckReadSoFarOrThrow;
    }
