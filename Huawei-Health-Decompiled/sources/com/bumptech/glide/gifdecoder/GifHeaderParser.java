    ...
    private void readNetscapeExt() {
        do {
            ...
            if (bArr[0] == 1) {
                byte b = bArr[1];
                byte b2 = bArr[2];
                this.header.loopCount = ((b2 & UnsignedBytes.MAX_VALUE) << 8) | (b & UnsignedBytes.MAX_VALUE);
            }
            if (this.blockSize <= 0) {
                return;
            }
    ...
    private int[] readColorTable(int i) {
        ...
        try {
            ...
            for (int i3 = 0; i3 < i; i3++) {
                byte b = bArr[i2];
                iArr[i3] = ((bArr[i2 + 1] & UnsignedBytes.MAX_VALUE) << 8) | ((b & UnsignedBytes.MAX_VALUE) << 16) | (-16777216) | (bArr[i2 + 2] & UnsignedBytes.MAX_VALUE);
                i2 += 3;
            }
    ...
    private void readBlock() {
        ...
        while (true) {
            try {
                ...
                i2 -= i3;
                this.rawData.get(this.block, i3, i2);
                i3 += i2;
            } catch (Exception e) {
                if (Log.isLoggable(TAG, 3)) {
                    Log.d(TAG, "Error Reading Block n: " + i3 + " count: " + i2 + " blockSize: " + this.blockSize, e);
                }
                this.header.status = 1;
                return;
            }
