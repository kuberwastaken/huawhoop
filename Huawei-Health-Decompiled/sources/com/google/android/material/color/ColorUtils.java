...
final class ColorUtils {
    ...
    public static int redFromInt(int i) {
        return (i & HwColorPicker.MASK_RESULT_STATE) >> 16;
    }
    ...
    public static int intFromXyzComponents(float f, float f2, float f3) {
        ...
        float f5 = f2 / 100.0f;
        float f6 = f3 / 100.0f;
        return intFromRgb(Math.max(Math.min(255, Math.round(delinearized((3.2406f * f4) + ((-1.5372f) * f5) + ((-0.4986f) * f6)) * 255.0f)), 0), Math.max(Math.min(255, Math.round(delinearized(((-0.9689f) * f4) + (1.8758f * f5) + (0.0415f * f6)) * 255.0f)), 0), Math.max(Math.min(255, Math.round(delinearized((f4 * 0.0557f) + (f5 * (-0.204f)) + (f6 * 1.057f)) * 255.0f)), 0));
    }
