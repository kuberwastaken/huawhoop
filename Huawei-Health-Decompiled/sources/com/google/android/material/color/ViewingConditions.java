...
final class ViewingConditions {
    ...
    static ViewingConditions make(float[] fArr, float f, float f2, float f3, boolean z) {
        ...
        float f16 = 1.0f - f15;
        float fCbrt = (0.1f * f16 * f16 * ((float) Math.cbrt(((double) f) * 5.0d))) + (f15 * f);
        ...
        float fPow2 = (float) Math.pow(((double) ((fArr6[0] * fCbrt) * f9)) / 100.0d, 0.42d);
        float fPow3 = (float) Math.pow(((double) ((fArr6[1] * fCbrt) * f10)) / 100.0d, 0.42d);
        float fPow4 = (float) Math.pow(((double) ((fArr6[2] * fCbrt) * f11)) / 100.0d, 0.42d);
        float[] fArr7 = {fPow2, fPow3, fPow4};
        float f17 = fArr7[0];
        float f18 = fArr7[1];
        return new ViewingConditions(fYFromLstar, ((((f17 * 400.0f) / (f17 + 27.13f)) * 2.0f) + ((f18 * 400.0f) / (f18 + 27.13f)) + (((400.0f * fPow4) / (fPow4 + 27.13f)) * 0.05f)) * fPow, fPow, fPow, f13, f12, fArr6, fCbrt, (float) Math.pow(fCbrt, 0.25d), fSqrt + 1.48f);
    }
