...
- The dashboard now adds interactive derived charts from current artifacts: signal mini-sparklines, stress/strain stacked bars, sleep-stage architecture ribbons, route coverage meters, watchface controls, a sandboxed Watchface Lab validator, weather controls, bridge network state, bridge command timeline, redacted export, and token-aware command posting.
